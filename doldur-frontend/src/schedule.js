function apply_criteria_courses(surname, department, grade, courses) {
    for (var i = courses.length - 1; i >= 0; i--) {
        courses[i] = apply_criteria_sections(
            surname,
            department,
            grade,
            courses[i]
...
function surnameCheck(surname, course_surname_start, course_surname_end) {
    ...
    if (
        surStartFirstVal === surFirstVal &&
        surFirstVal < surEndFirstVal &&
            ...
            for (var j = 0; j < course.sections[i].criteria.length; j++) {
                ...
                if (course.checkSurname === false) {
                    ...
                    if (
                        surnameCheck(
                            surname,
                            criterion.surnameStart,
                            criterion.surnameEnd
                        ) === true
                    ) {
                        surn_passed = true;
                    }
...
function lectures_intersect(lt1, lt2) {
    if (
        lt1.day !== lt2.day || // Different Days
...
export function compute_schedule(
    surname,
    department,
    grade,
    courses,
...
function recursive_computation(
    courses,
    dontFills,
    depth,
    scenario,
    ...
    for (var i = 0; i < courses[depth].sections.length; i++) {
        ...
        for (var j = 0; j < scenario.length; j++) {
            if (
                courses[depth].checkCollision == true &&
                scenario[j].collision == true &&
                check_collision(
                    courses[depth].sections[i],
                    scenario[j].section
                ) === true
            ) {
                collision = true;
        ...
        for (var j = 0; j < dontFills.length; j++) {
            if (
                check_collision_df(courses[depth].sections[i], dontFills[j]) ===
                true
            ) {
                collision = true;
            }
