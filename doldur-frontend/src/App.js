...
class App extends React.Component {
    ...
    handleDontFillDelete(startDate) {
        this.setState({
            dontFills: this.state.dontFills.filter(
                (df) => df.startDate !== startDate
            ),
        });
    }
                    ...
                    <div className={isMobile ? "column" : "row"}>
                        <WeeklyProgram
                            ...
                            onChangeDontFillColor={(startDate, color) =>
                                this.handleChangeDontFillColor(startDate, color)
                            }
                            ...
                            onDontFillDelete={(startDate) =>
                                this.handleDontFillDelete(startDate)
                            }
                        />
                        <Controls
                            onSchedule={(s) => this.setState({ scenarios: s })}
                            dontFills={this.state.dontFills}
                            onDontFillAdd={(startDate, endDate, desc) =>
                                this.handleDontFillAdd(startDate, endDate, desc)
                            }
