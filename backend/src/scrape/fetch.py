...
def get_main_page(session: requests.Session) -> requests.Response:
    response = http_get(session, app_constants.oibs64_url, name="main_page")
    response.encoding = "utf-8"
    return response


def get_dept(session: requests.Session, dept_code: str, semester_code: str, tries: int = 5):
    data = {
        "textWithoutThesis": 1,
    ...
    response = post_oibs(session, data, tries=tries, base_delay=0.9, name="get_dept")
    response.encoding = "utf-8"
    return response


def get_course(session: requests.Session, course_code: str, tries: int = 5):
    data = {
        "SubmitCourseInfo": "Course Info",
        "text_course_code": course_code,
        "hidden_redir": "Course_List",
    }
    response = post_oibs(session, data, tries=tries, base_delay=0.9, name="get_course")
    response.encoding = "utf-8"
    return response


def get_section(session: requests.Session, section_code: str, tries: int = 5):
    data = {"submit_section": section_code, "hidden_redir": "Course_Info"}
    response = post_oibs(session, data, tries=tries, base_delay=0.9, name="get_section")
    response.encoding = "utf-8"
    return response
