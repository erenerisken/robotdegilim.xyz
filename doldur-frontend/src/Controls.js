...
export class Controls extends React.Component {
  ...
  handleAddMustCourse() {
    ...
    getMusts(this.state.department, this.state.semester)
      ...
      .catch((_) => {
        this.setState({
          alertMsg: "Must courses for your department are not available",
          errorDept: true,
        });
      });
  }
  ...
  handleScheduleBegin() {
    ...
    this.state.selectedCourses.map((c) => {
      ...
      const courseToPush = {
        ...
        checkCollision:
          this.state.settings.checkCollision && c.settings.checkCollision,
        checkDepartment:
          this.state.settings.checkDepartment && c.settings.checkDepartment,
        sections: Array(0),
      };
        ...
        <div className={"control-row"}>
          <div className={"textfield-wrapper"}>
            <TextField
              ...
              size={"small"}
              onChange={(e) =>
                this.setState({ surname: e.target.value.toUpperCase() })
              }
            />
          </div>
          <div className={"textfield-wrapper"}>
          ...
          {this.state.selectedCourses.map((c, i) => {
            return c !== null ? (
              <CourseCard
                ...
                onColorChange={(c) => this.handleCourseColor(i, c)}
              />
            ) : null;
          })}
        </div>
